...
def _cells_of(values, threshold, grid, cells):
    if cells is not None:
        return (np.asarray(cells[0], float), np.asarray(cells[1], float),
                np.asarray(cells[2], float))
...
def profile_ci_coef(
    ...
    b_hat = float(np.asarray(fit["coef"], float)[coef])
...
def profile_ci_return_level(
    ...
    f = fit_grouped_design(values, threshold, np.ones(values.size)[:, None],
                           cells=(a, b, trunc))
